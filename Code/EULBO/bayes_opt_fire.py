...
def ELBO_simulations(
    ...
    for sim in range(n_simulations):
        ...
        for epoch in tqdm(range(n_epochs), leave = False):
            ...
            optimizer_S = torch.optim.Adam(params = [ls_opt, os_opt, sigma_sq_opt], lr = 0.005, maximize = True)
            for _ in range(500):
                ...
                try:
                    ...
                    try:
                        STKS_chol = torch.linalg.cholesky(KXX + 1e-6 * torch.eye(X.shape[-2]))
                    except:
                        break   
...
def BO_Fire( 
    ...
    # Run simulations
    simulation_dict = ELBO_simulations(D, N, N, truenoise, n_simulations, n_epochs)
    sim_df = pd.DataFrame(simulation_dict)

    # Count existing number of csv files
    n_csvs = len(fnmatch.filter(oper.listdir('./Code/EULBO/Sim-Results/BayesOpt/'), '*.csv'))

    # Save file locally
    sim_df.to_csv(f"./Code/EULBO/Sim-Results/BayesOpt/BayesOpt_Simulation_Results_{n_csvs}.csv",
                  index = False)
